...
developers of 11 packages (`amt`, `crawl`, `gpx`, `lsmnsd`, `moveHMM`,
`moveNT`, `spatsoc`,`trackeR`, `VTrack`, `wildlifeDI`, `wildxing`),
who shown interest in working together. Altogether, feedback received
only confirmed the need for a central movement package in R. We will
keep working in the open, and collect feedback from the whole R
movement community.
...
* **Integration of `strack`** [month 2–4]: In parallel, we will work
  with developers willing to fully integrate the solution offered by
  `sftrack` at the core of their data flow. This step may require
...
* **Submission to rOpenSci** [month 5–6]: In the last two months, we
  ...
  rOpenSci, which will coincide with the stable API of version
  1.0.0. rOpenSci emphasizes (and basically enforces) a package's
...
While the work on `sftrack` will happen completely in the open, we
will keep communicating our progress, directly on Twitter as well as
with more detailed blog posts on https://mablab.org/. We will watch
for feedback, notably through the GitHub repository, which will
collect use cases, bug issues and suggestions for enhancements. We
have already successfully presented our work on `sftrack` at the 2020
Moving2Gather conference (held in Rennes, France) in March 2020), and
will certainly consider other venues once the situation with COVID-19
allows us to normally travel.
